            ...
            try
            {
                ...
                if (hashEntries.Count() == 2)
                {
                    ...
                    else
                    {
                        ...
                        {
                            //_logger.LogError
                            //模型变更可能导致二进制序列化异常，清除数据
                            await _db.KeyDeleteAsync(key);
                            grainState.State = null;//赋值null,基础类根据null判断是否从数据库里重新加载数据
                            _logger.LogError("二进制序列化异常: GrainType={0} PrimaryKey={1} Grainid={2} ETag={3} from Database={4} Error: {5}",
                                grainType, key, grainReference, grainState.ETag, _db.Database, ex.Message);
                        }
                    }
